                ...
                @foreach($sweets as $sweet)
                    <article>
                        ...
                        <div class="ml-4 text-sm text-gray-500 sm:ml-0">
                            <form action="{{ route('like', ['model_type' => \App\Models\Sweet::class, 'model_id' => $sweet->id]) }}" method="POST">
                                @csrf

                                <button type="submit" class="bg-gray-200 text-gray-600 hover:bg-gray-300 px-4 py-2 rounded-full">
                                    @if(auth()->user()->likes()->where('likeable_id', $sweet->id)->where('likeable_type', 'sweet')->first())
                                        liked
                                    @else
                                        like
                                    @endif
